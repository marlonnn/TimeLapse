...
namespace TimeLapse.SystemFunction
{
    public class MessageBoxManager
    {
        ...
        public static int YesXDelta = 0;    // x delta of Yes button, for long text string of yes button

        public static int NoXDelta = 0; // x delta of No button, for long text string of no button

        /// <summary>
        /// reset text according to ui language
        /// </summary>
        public static void Reset()
        {
            Unregister();

            bool bZhCn = Thread.CurrentThread.CurrentUICulture.Name == "zh-CN";

            OK = bZhCn ? "确定" : "OK";

            Cancel = bZhCn ? "取消" : "Cancel";

            Abort = bZhCn ? "中止(&A)" : "&Abort";

            Retry = bZhCn ? "重试(&R)" : "&Retry";

            Ignore = bZhCn ? "忽略(&I)" : "&Ignore";
        ...
        private static void AdjustDlgItemXPosition(IntPtr dlg, int itemId, int delatX, int delatWidth)
        {
            NativeMethods.POINT point = new NativeMethods.POINT();
            NativeMethods.RECT rect = new NativeMethods.RECT();

            IntPtr dlgItem = GetDlgItem(dlg, itemId);
            if (dlgItem != IntPtr.Zero)
            {
                NativeMethods.GetWindowRect(dlgItem, ref rect); // get original position
                point.x = rect.left;
                point.y = rect.top;
                NativeMethods.ScreenToClient(dlg, ref point);   // get client position
                // set new position
                NativeMethods.SetWindowPos(dlgItem, IntPtr.Zero,
                    point.x - delatX, point.y, rect.Size.Width + delatWidth, rect.Size.Height, NativeMethods.SWP_NOZORDER);
            }
        }
